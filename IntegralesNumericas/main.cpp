...
int intChecker(){
    int x; // DeclaraciÑn de la variable x para almacenar el nÏmero entero ingresado
    while (true) // Bucle infinito para repetir la solicitud de entrada hasta que se ingrese un nÏmero entero positivo vàlido
    {
        std::string u; // DeclaraciÑn de la variable u para almacenar la entrada del usuario
        ...
        try // Bloque try-catch para manejar excepciones
        {
            ...
            char test; // Variable para almacenar un caràcter adicional
            if ((!(stream >> x)) || (stream >> test)) // Verificar si la entrada se puede convertir a un nÏmero entero y si no hay caracteres adicionales
            {
                throw std::runtime_error("Failed"); // Lanzar una excepciÑn si la conversiÑn falla o si hay caracteres adicionales
            }
            if (x >= 0) // Verificar si el nÏmero entero es cero o positivo
            {
                return x; // Devolver el nÏmero entero positivo
            }
            else
            {
                Error("Ponga un nÏmero entero positivo"); // Imprimir un mensaje de error si el nÏmero entero no es positivo
            }
        }
        catch(...) // Manejar cualquier tipo de excepciÑn
        {
            Error("Ponga un nÏmero entero"); // Imprimir un mensaje de error general
        }
    }
}
